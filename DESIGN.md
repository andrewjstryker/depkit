...
It manages **CSS and JavaScript assets** produced by R tooling (notably
`htmlwidgets`) and provides a deterministic, usage-agnostic way to:
...
- **emit HTML snippets** (`<link>`, `<script>`) for callers to insert
  into documents they are constructing.

depkit **does not** write HTML files, track what has been emitted, or
impose document layout policy.
...
Where `rel_path` is the stylesheet or script path declared by the
dependency.

Asset ids are: - stable - output-agnostic - the sole unit tracked by the
dependency manager
...
The `DependencyManager` (DM) holds the **only persistent state** in
depkit:
...
- `cdn` — logical; automatic CDN resolution via jsDelivr (JavaScript
  only)

Configuration affects copying and emission, but is **not stateful** in
the sense of “prepared” or “emitted”.
...
> **DM never tracks whether assets were emitted or prepared.** It only
> tracks **what assets exist**, in order.
...
- `cdn = TRUE`
  ...
  - At insert time, query jsDelivr API for the npm package matching the
...
Callers decide *where* snippets go. depkit decides *what* those snippets
are.
